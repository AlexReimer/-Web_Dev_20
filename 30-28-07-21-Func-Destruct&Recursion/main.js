...
// c 6-го JS не нужно писать текст-замену как выше 
/* "if(text === undefined){
        text = 'text not added'" */
//теперь можно прописать сразу, что выводся, если параметры пустые 
function showMessage(from ='no name', text = 'empty text'){
    console.log(`${from}: ${text} `)
}
showMessage('Vasya', 'Nice to meet you!');

showMessage(); //undefined выдаст

function calc(a=2,b=3){
    return a + b;
}

console.log(calc()); // -> 5 //возьмет, то что передано в сигнатуре функции: function calc(a=2,b=3)
console.log(calc(5)); // -> 8 //возьмет 5, а второй аргумент из сигнатуры функции b=3 
console.log(calc(4,5)); // -> 9 //сложит точто пеередано тут 
